...
// This script tests the complete payment verification process
...
async function testPaymentVerification() {
  ...
  if (pendingAuctions.length === 0) {
    console.log('❌ No PENDING_PAYMENT auctions found. Let\'s create a test auction first.')
    return
  }

  // 2. Create barracks items for PENDING_PAYMENT auctions
  ...
  for (const auction of pendingAuctions) {
    ...
    const { data: barracksItem, error: createError } = await supabase
      ...
      .insert({
        ...
        amount_cents: auction.current_bid_id ? 1000 : 500, // Default amounts for testing
        status: 'PENDING_PAYMENT',
        payment_id: `test_payment_${auction.id}` // Mock payment ID
      })
      .select()
      .single()
  ...
  // 4. Test payment verification (simulate the cron job)
  ...
  if (fetchError) {
    console.log('❌ Error fetching pending items:', fetchError.message)
    return
  }

  console.log(`Found ${pendingItems.length} PENDING_PAYMENT items to verify`)

  let verifiedCount = 0
  for (const item of pendingItems) {
    try {
      // Simulate payment verification (in real flow, this would check Whop API)
      if (item.payment_id) {
        ...
        if (updateError) {
          console.log(`❌ Error updating barracks item ${item.id}:`, updateError.message)
          continue
        }
        ...
        if (auctionError) {
          console.log(`❌ Error updating auction ${item.auction_id}:`, auctionError.message)
          continue
        }
  ...
  console.log(`\n🎉 Payment verification completed! ${verifiedCount} items verified.`)
