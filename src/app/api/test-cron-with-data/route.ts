...
export async function POST(request: NextRequest) {
  try {
    console.log('🧪 Testing cron job with test data...')
    ...
    const { data: testAuction, error: createError } = await supabaseServer
      .from('auctions')
      .insert({
        title: '🧪 Test Auction for Cron Job',
        description: 'This auction is created to test the cron job functionality',
    ...
    return NextResponse.json({
      ...
      instructions: [
        ...
        '2. Call the cron job endpoint again',
        '3. Check if the auction status changes to PENDING_PAYMENT'
      ]
    })

  } catch (error) {
    console.error('❌ Test cron with data failed:', error)
